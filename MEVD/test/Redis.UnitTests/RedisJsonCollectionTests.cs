...
namespace CommunityToolkit.VectorData.Redis.UnitTests;
...
public class RedisJsonCollectionTests
{
    ...
    public async Task CollectionExistsReturnsCollectionStateAsync(string collectionName, bool expectedExists)
    {
        ...
        this._redisDatabaseMock
            .Verify(
                x => x.ExecuteAsync(
                    "FT.INFO",
                    It.Is<ICollection<object>>(x => x.SequenceEqual(expectedArgs)), It.IsAny<CommandFlags>()),
                Times.Once);
    ...
    public async Task CanDeleteManyRecordsWithVectorsAsync(bool useDefinition)
    {
        ...
        this._redisDatabaseMock
            .Verify(
                x => x.ExecuteAsync(
                    "JSON.DEL",
                    It.Is<ICollection<object>>(x => x.SequenceEqual(expectedArgs1)), It.IsAny<CommandFlags>()),
                Times.Once);
        this._redisDatabaseMock
            .Verify(
                x => x.ExecuteAsync(
                    "JSON.DEL",
                    It.Is<ICollection<object>>(x => x.SequenceEqual(expectedArgs2)), It.IsAny<CommandFlags>()),
                Times.Once);
    }
    ...
    public async Task CanSearchWithVectorAndFilterAsync(bool useDefinition)
    {
        ...
        this._redisDatabaseMock
            .Verify(
                x => x.ExecuteAsync(
                    "FT.SEARCH",
                    It.Is<ICollection<object>>(x => x.Where(y => !(y is byte[])).SequenceEqual(expectedArgs.Where(y => !(y is byte[])))), It.IsAny<CommandFlags>()),
                Times.Once);
    ...
    private static void SetupExecuteMock(Mock<IDatabase> redisDatabaseMock, Exception exception)
    {
        redisDatabaseMock
            .Setup(
                x => x.ExecuteAsync(
                    It.IsAny<string>(),
                    It.IsAny<ICollection<object>>(), It.IsAny<CommandFlags>()))
    ...
    public sealed class MultiPropsModel
    {
        ...
        [VectorStoreVector(dimensions: 4, DistanceFunction = DistanceFunction.CosineDistance, StorageName = "ignored_vector1_storage_name")]

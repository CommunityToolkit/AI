...
internal sealed class CosmosDBTestStore(string uniqueDatabaseName) : TestStore
{
    ...
    {
        PropertyNamingPolicy = new CosmosDBTestJsonNamingPolicy()
    };
    ...
    {
        if (CosmosDBTestEnvironment.IsConnectionStringDefined)
        {
            _connectionString = CosmosDBTestEnvironment.ConnectionString!;
        }
        else
        {
        ...
        DefaultVectorStore = new CosmosDBVectorStore(_database, new() { JsonSerializerOptions = SerializerOptions });
    }
    ...
    private sealed class CosmosDBTestJsonNamingPolicy : JsonNamingPolicy
    {

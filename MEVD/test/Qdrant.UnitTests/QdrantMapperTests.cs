...
public class QdrantMapperTests
{
    ...
    public void MapsSinglePropsFromDataToStorageModelWithUlong(bool hasNamedVectors)
    {
        ...
        if (hasNamedVectors)
        {
            Assert.Equal(new float[] { 1, 2, 3, 4 }, actual.Vectors.Vectors_.Vectors["vector"].Dense.Data.ToArray());
        }
        else
        {
            Assert.Equal(new float[] { 1, 2, 3, 4 }, actual.Vectors.Vector.Dense.Data.ToArray());
        }
    }
    ...
    public void MapsMultiPropsFromDataToStorageModelWithUlong()
    {
        ...
        Assert.Equal(new float[] { 1, 2, 3, 4 }, actual.Vectors.Vectors_.Vectors["vector1"].Dense.Data.ToArray());
        Assert.Equal(new float[] { 5, 6, 7, 8 }, actual.Vectors.Vectors_.Vectors["vector2"].Dense.Data.ToArray());
    }
    ...
    private sealed class SinglePropsModel<TKey>
    {
        ...
        [VectorStoreVector(dimensions: 10, StorageName = "vector")]
    ...
    private sealed class MultiPropsModel<TKey>
    {
        ...
        [VectorStoreVector(dimensions: 10, StorageName = "vector1")]
        public ReadOnlyMemory<float>? Vector1 { get; set; }

        [VectorStoreVector(dimensions: 10, StorageName = "vector2")]

...
internal sealed class PineconeTestStore : TestStore
{
    ...
    private const int IndexServiceCount = 20;
    ...
    protected override async Task StartAsync()
    {
        ...
        this._clientOptions = new()
        {
            ...
            MaxRetries = 3,
            IsTlsEnabled = false,
            // The Pinecone SDK only retries on HTTP status codes (408/429/500+), not on connection failures.
            // The Pinecone local container can reset idle connections between test class executions, so we
            // use a retry handler that catches HttpRequestException (connection reset) and retries.
            HttpClient = new(new RetryHandler())
            {
    ...
    /// <summary>
    /// Retries on <see cref="HttpRequestException"/> (connection reset by peer) which the Pinecone SDK
    /// doesn't handle — its built-in retry only covers HTTP status codes.
    /// </summary>
    private sealed class RetryHandler() : DelegatingHandler(new SocketsHttpHandler())
    {

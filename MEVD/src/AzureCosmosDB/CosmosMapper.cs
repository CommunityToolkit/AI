...
/// <summary>
/// Class for mapping between a json node stored in Azure Cosmos NoSQL and the consumer data model.
/// </summary>
/// <typeparam name="TRecord">The consumer data model to map to or from.</typeparam>
internal sealed class CosmosMapper<TRecord> : ICosmosMapper<TRecord>
    where TRecord : class
{
    ...
    public CosmosMapper(CollectionModel model, JsonSerializerOptions? jsonSerializerOptions)
    {
        this._model = model;
    ...
    public JsonObject MapFromDataToStorageModel(TRecord dataModel, int recordIndex, IReadOnlyList<MEAI.Embedding>?[]? generatedEmbeddings)
    {
        ...
        // The key property in Azure Cosmos NoSQL is always named 'id'.
        ...
        RenameJsonProperty(jsonObject, this._keyProperty.SerializedKeyName!, CosmosConstants.ReservedKeyPropertyName);
        ...
        for (var i = 0; i < this._model.VectorProperties.Count; i++)
        {
            ...
            object? embedding = generatedEmbeddings?[i]?[recordIndex];
            ...
            if (embedding is null)
            {
                switch (Nullable.GetUnderlyingType(property.Type) ?? property.Type)
                {
                    ...
                    case var t2 when t2 == typeof(Embedding<sbyte>):
                        embedding = property.GetValueAsObject(dataModel);
                        break;

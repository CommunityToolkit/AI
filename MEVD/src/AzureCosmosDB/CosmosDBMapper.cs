...
internal sealed class CosmosDBMapper<TRecord> : ICosmosDBMapper<TRecord>
    where TRecord : class
{
    ...
    public CosmosDBMapper(CollectionModel model, JsonSerializerOptions? jsonSerializerOptions)
    {
        this._model = model;
    ...
    public JsonObject MapFromDataToStorageModel(TRecord dataModel, int recordIndex, IReadOnlyList<MEAI.Embedding>?[]? generatedEmbeddings)
    {
        ...
        RenameJsonProperty(jsonObject, this._keyProperty.SerializedKeyName!, CosmosDBConstants.ReservedKeyPropertyName);
    ...
    {
        // See above comment.
        RenameJsonProperty(storageModel, CosmosDBConstants.ReservedKeyPropertyName, this._keyProperty.SerializedKeyName!);

...
internal class CosmosModelBuilder() : CollectionJsonModelBuilder(s_modelBuildingOptions)
{
    ...
    private static readonly CollectionModelBuildingOptions s_modelBuildingOptions = new()
    {
        ...
        ReservedKeyStorageName = CosmosConstants.ReservedKeyPropertyName
    };

    protected override void ValidateKeyProperty(KeyPropertyModel keyProperty)
    {
        // CosmosKey is a composite key type (document ID + partition key) that doesn't correspond to any single key property type;
        // skip the base TKey-to-key-property validation when it's used.
        if (this.KeyType != typeof(CosmosKey))
        {
            base.ValidateKeyProperty(keyProperty);
        }

        // Note that the key property in Cosmos NoSQL refers to the document ID, not to the CosmosKey structure which includes both

...
/// </summary>
internal sealed class CosmosDBDynamicMapper(CollectionModel model, JsonSerializerOptions jsonSerializerOptions)
    : ICosmosDBMapper<Dictionary<string, object?>>
{
    public JsonObject MapFromDataToStorageModel(Dictionary<string, object?> dataModel, int recordIndex, IReadOnlyList<MEAI.Embedding>?[]? generatedEmbeddings)
    {
        ...
        {
            [CosmosDBConstants.ReservedKeyPropertyName] = !dataModel.TryGetValue(model.KeyProperty.ModelName, out var keyValue)
        ...
        for (var i = 0; i < model.VectorProperties.Count; i++)
        {
            ...
            {
                var vector = (object?)generatedEmbeddings?[i]?[recordIndex] ?? vectorValue;
        ...
        foreach (var property in model.Properties)
        {
            switch (property)
            {
                case KeyPropertyModel keyProperty:
                    var key = (string?)storageModel[CosmosDBConstants.ReservedKeyPropertyName]

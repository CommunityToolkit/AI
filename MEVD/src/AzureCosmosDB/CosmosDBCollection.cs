...
///     is the document ID), and <see cref="CosmosDBKey"/> (for other partition key configurations).</typeparam>
/// <typeparam name="TRecord">The data model to use for adding, updating and retrieving data from storage.</typeparam>
#pragma warning disable CA1711 // Identifiers should not have incorrect suffix
public class CosmosDBCollection<TKey, TRecord> : VectorStoreCollection<TKey, TRecord>, IKeywordHybridSearchable<TRecord>
    where TKey : notnull
    where TRecord : class
#pragma warning restore CA1711 // Identifiers should not have incorrect suffix
{
    ...
    private readonly ICosmosDBMapper<TRecord> _mapper;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CosmosDBCollection{TKey, TRecord}"/> class.
    /// </summary>
    /// <param name="database"><see cref="Database"/> that can be used to manage the collections in Azure CosmosDB NoSQL.</param>
    /// <param name="name">The name of the collection that this <see cref="CosmosDBCollection{TKey, TRecord}"/> will access.</param>
    ...
    public CosmosDBCollection(Database database, string name, CosmosDBCollectionOptions? options = default)
    ...
    internal CosmosDBCollection(
        ...
        : this(
            ...
            static options => typeof(TRecord) == typeof(Dictionary<string, object?>)
                ? throw new NotSupportedException(VectorDataStrings.NonDynamicCollectionWithDictionaryNotSupported(typeof(CosmosDBDynamicCollection)))
                : new CosmosDBModelBuilder()
    ...
    internal CosmosDBCollection(
        ...
        Func<CosmosClient, Database> databaseProvider,
        string name,
        Func<CosmosDBCollectionOptions, CollectionModel> modelFactory,
        CosmosDBCollectionOptions? options)
    {
        // Validate TKey: must be string or Guid (partition key = document ID), CosmosDBKey (other partition key cases), or object (used by CosmosDBDynamicCollection).
        if (typeof(TKey) != typeof(string) && typeof(TKey) != typeof(Guid) && typeof(TKey) != typeof(CosmosDBKey) && typeof(TKey) != typeof(object))
        {
            throw new NotSupportedException($"The key type must be string, Guid, or CosmosDBKey. Received: {typeof(TKey).Name}");
        }
        ...
        try
        {
            ...
            options ??= CosmosDBCollectionOptions.Default;
            ...
            this._mapper = typeof(TRecord) == typeof(Dictionary<string, object?>)
                ? (new CosmosDBDynamicMapper(this._model, jsonSerializerOptions) as ICosmosDBMapper<TRecord>)!
                : new CosmosDBMapper<TRecord>(this._model, options.JsonSerializerOptions);
            ...
            // must use CosmosDBKey as the key type.
                ...
                && (this._partitionKeyProperties is not [var singlePkProperty] || singlePkProperty != this._model.KeyProperty))
            {
                throw new ArgumentException(
                    ...
                    $"('{this._model.KeyProperty.ModelName}'). To use a different partition key configuration, use CosmosDBKey as the key type.");
            }

            this._collectionMetadata = new()
            {
                VectorStoreSystemName = CosmosDBConstants.VectorStoreSystemName,
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < vectorPropertyCount; i++)
        {
            ...
            if (CosmosDBModelBuilder.IsVectorPropertyTypeValidCore(vectorProperty.Type, out _))
            {
                continue;
            }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var queryDefinition = CosmosDBCollectionQueryBuilder.BuildSearchQuery(
            vector,
            null,
            this._model,
    ...
    private static async ValueTask<object> GetSearchVectorAsync<TInput>(TInput searchValue, VectorPropertyModel vectorProperty, CancellationToken cancellationToken)
        ...
        => searchValue switch
        {
            ...
            _ => vectorProperty.EmbeddingGenerator is null
                ? throw new NotSupportedException(VectorDataStrings.InvalidSearchInputAndNoEmbeddingGeneratorWasConfigured(searchValue.GetType(), CosmosDBModelBuilder.SupportedVectorTypes))
        ...
        FilteredRecordRetrievalOptions<TRecord>? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var (whereClause, filterParameters) = new CosmosDBFilterTranslator().Translate(filter, this._model);
        ...
        where TInput : notnull
    {
        ...
        var queryDefinition = CosmosDBCollectionQueryBuilder.BuildSearchQuery<TRecord>(
            vector,
            keywords,
            this._model,
            vectorProperty.StorageName,
    ...
    private void VerifyVectorType<TVector>(TVector? vector)
    {
        ...
        if (!CosmosDBModelBuilder.IsVectorPropertyTypeValidCore(vectorType, out var supportedTypes))
        {
    ...
    private (string DocumentId, PartitionKey PartitionKey) GetDocumentIdAndPartitionKey(TKey key)
        => key switch
        {
            CosmosDBKey cosmosKey => (cosmosKey.DocumentId, cosmosKey.PartitionKey),
            string s => (s, new PartitionKey(s)),
            Guid g => (g.ToString(), new PartitionKey(g.ToString())),
            _ => throw new InvalidOperationException(
                $"Keys must be of type string, Guid, or CosmosDBKey. Received key of type '{key.GetType().FullName}'.")
        };

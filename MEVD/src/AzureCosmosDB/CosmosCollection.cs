...
/// <summary>
/// Service for storing and retrieving vector records, that uses Azure Cosmos NoSQL as the underlying storage.
/// </summary>
/// <typeparam name="TKey">
///     The data type of the record key. Supported types are <see cref="string"/> and <see cref="Guid"/> (in which case the partition key
///     is the document ID), and <see cref="CosmosKey"/> (for other partition key configurations).</typeparam>
/// <typeparam name="TRecord">The data model to use for adding, updating and retrieving data from storage.</typeparam>
#pragma warning disable CA1711 // Identifiers should not have incorrect suffix
public class CosmosCollection<TKey, TRecord> : VectorStoreCollection<TKey, TRecord>, IKeywordHybridSearchable<TRecord>
    where TKey : notnull
    where TRecord : class
#pragma warning restore CA1711 // Identifiers should not have incorrect suffix
{
    ...
    /// <summary><see cref="Database"/> that can be used to manage the collections in Azure Cosmos NoSQL.</summary>
    ...
    /// <summary>The mapper to use when mapping between the consumer data model and the Azure Cosmos NoSQL record.</summary>
    private readonly ICosmosMapper<TRecord> _mapper;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="CosmosCollection{TKey, TRecord}"/> class.
    /// </summary>
    /// <param name="database"><see cref="Database"/> that can be used to manage the collections in Azure Cosmos NoSQL.</param>
    /// <param name="name">The name of the collection that this <see cref="CosmosCollection{TKey, TRecord}"/> will access.</param>
    ...
    public CosmosCollection(Database database, string name, CosmosCollectionOptions? options = default)
    ...
    internal CosmosCollection(
        ...
        : this(
            ...
            static options => typeof(TRecord) == typeof(Dictionary<string, object?>)
                ? throw new NotSupportedException(VectorDataStrings.NonDynamicCollectionWithDictionaryNotSupported(typeof(CosmosDynamicCollection)))
    ...
    internal CosmosCollection(
        ...
        Func<CosmosClient, Database> databaseProvider,
        string name,
        Func<CosmosCollectionOptions, CollectionModel> modelFactory,
        CosmosCollectionOptions? options)
    {
        // Validate TKey: must be string or Guid (partition key = document ID), CosmosKey (other partition key cases), or object (used by CosmosDynamicCollection).
        if (typeof(TKey) != typeof(string) && typeof(TKey) != typeof(Guid) && typeof(TKey) != typeof(CosmosKey) && typeof(TKey) != typeof(object))
        {
            throw new NotSupportedException($"The key type must be string, Guid, or CosmosKey. Received: {typeof(TKey).Name}");
        }
        ...
        try
        {
            ...
            options ??= CosmosCollectionOptions.Default;
            ...
            this._mapper = typeof(TRecord) == typeof(Dictionary<string, object?>)
                ? (new CosmosDynamicMapper(this._model, jsonSerializerOptions) as ICosmosMapper<TRecord>)!
                : new CosmosMapper<TRecord>(this._model, options.JsonSerializerOptions);
            ...
            {
                VectorStoreSystemName = CosmosConstants.VectorStoreSystemName,
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < vectorPropertyCount; i++)
        {
            ...
            if (CosmosModelBuilder.IsVectorPropertyTypeValidCore(vectorProperty.Type, out _))
            {
                continue;
            }
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var queryDefinition = CosmosCollectionQueryBuilder.BuildSearchQuery(
            vector,
            null,
            this._model,
    ...
    private static async ValueTask<object> GetSearchVectorAsync<TInput>(TInput searchValue, VectorPropertyModel vectorProperty, CancellationToken cancellationToken)
        ...
        => searchValue switch
        {
            ...
            _ => vectorProperty.EmbeddingGenerator is null
                ? throw new NotSupportedException(VectorDataStrings.InvalidSearchInputAndNoEmbeddingGeneratorWasConfigured(searchValue.GetType(), CosmosModelBuilder.SupportedVectorTypes))
        ...
        FilteredRecordRetrievalOptions<TRecord>? options = null, [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var (whereClause, filterParameters) = new CosmosFilterTranslator().Translate(filter, this._model);
        ...
        where TInput : notnull
    {
        ...
        var queryDefinition = CosmosCollectionQueryBuilder.BuildSearchQuery<TRecord>(
            vector,
            keywords,
            this._model,
            vectorProperty.StorageName,
    ...
    private void VerifyVectorType<TVector>(TVector? vector)
    {
        ...
        if (!CosmosModelBuilder.IsVectorPropertyTypeValidCore(vectorType, out var supportedTypes))
        {
            throw new NotSupportedException(
                $"The provided vector type {vectorType.FullName} is not supported by the Azure Cosmos NoSQL connector. Supported types are: {supportedTypes}");
        }
    }
    ...
    private (string DocumentId, PartitionKey PartitionKey) GetDocumentIdAndPartitionKey(TKey key)
        => key switch
        {
            CosmosKey cosmosKey => (cosmosKey.DocumentId, cosmosKey.PartitionKey),
            string s => (s, new PartitionKey(s)),
            Guid g => (g.ToString(), new PartitionKey(g.ToString())),
            _ => throw new InvalidOperationException(
                $"Keys must be of type string, Guid, or CosmosKey. Received key of type '{key.GetType().FullName}'.")
        };
    ...
    private ContainerProperties GetContainerProperties()
    {
        ...
        var embeddings = new Collection<Microsoft.Azure.Cosmos.Embedding>();
        ...
        foreach (var property in this._model.VectorProperties)
        {
            ...
            var embedding = new Microsoft.Azure.Cosmos.Embedding
            {
    ...
    /// <summary>
    /// More information about Azure Cosmos NoSQL index kinds here: <see href="https://learn.microsoft.com/azure/cosmos-db/nosql/vector-search#vector-indexing-policies" />.
    /// </summary>
    private static VectorIndexType GetIndexKind(string? indexKind, string vectorPropertyName)
        => indexKind switch
        {
            "DiskAnn" or null => VectorIndexType.DiskANN,
            "Flat" => VectorIndexType.Flat,
            "QuantizedFlat" => VectorIndexType.QuantizedFlat,
            _ => throw new InvalidOperationException($"Index kind '{indexKind}' on {nameof(VectorStoreVectorProperty)} '{vectorPropertyName}' is not supported by the Azure Cosmos NoSQL VectorStore.")
        };

    /// <summary>
    /// More information about Azure Cosmos NoSQL distance functions here: <see href="https://learn.microsoft.com/azure/cosmos-db/nosql/vector-search#container-vector-policies" />.
    /// </summary>
    private static DistanceFunction GetDistanceFunction(string? distanceFunction, string vectorPropertyName)
        => distanceFunction switch
        {
            ...
            _ => throw new NotSupportedException($"Distance function '{distanceFunction}' for {nameof(VectorStoreVectorProperty)} '{vectorPropertyName}' is not supported by the Azure Cosmos NoSQL VectorStore.")
        };
    ...
    private static VectorDataType GetDataType(Type vectorDataType, string vectorPropertyName)
        => (Nullable.GetUnderlyingType(vectorDataType) ?? vectorDataType) switch
        {
            ...
            _ => throw new InvalidOperationException($"Data type '{vectorDataType}' for {nameof(VectorStoreVectorProperty)} '{vectorPropertyName}' is not supported by the Azure Cosmos NoSQL VectorStore.")
        };

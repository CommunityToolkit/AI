    ...
    where TRecord : class
{
        ...
        InMemoryCollectionOptions? options)
    {
        ...
        _internalCollections = internalCollection ?? new();
        _internalCollectionTypes = internalCollectionTypes ?? new();
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var filteredRecords = options.Filter is not null
            ? allValues.AsQueryable().Where(ConvertFilter(options.Filter))
            : allValues;

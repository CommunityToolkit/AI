...
internal class CosmosDocumentDBFilterTranslator : FilterTranslatorBase
{
    ...
    private BsonDocument TranslateEqualityComparison(BinaryExpression binary)
        => TryBindProperty(binary.Left, out var property) && binary.Right is ConstantExpression { Value: var rightConstant }
    ...
    private BsonDocument GenerateEqualityComparison(PropertyModel property, object? value, ExpressionType nodeType)
    {
        if (value is null)
        {
            throw new NotSupportedException("Azure DocumentDB does not support null checks in vector search pre-filters");
        }
    ...
    private BsonDocument TranslateNot(UnaryExpression not)
    {
        ...
        throw new NotSupportedException("Azure DocumentDB does not support the NOT operator in vector search pre-filters");
    }
